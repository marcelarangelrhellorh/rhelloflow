...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { password, expires_at, max_submissions, note } = body;
    ...
    if (error) {
      console.error('Error creating talent pool link:', error);
    ...
    console.log(`Talent pool link created: ${link.id}`);

    return new Response(JSON.stringify({
      id: link.id,
      token: link.token,
      url: `${req.headers.get('origin')}/banco-talentos/${link.token}`,
    ...
    console.error('Error in generate-talent-pool-link:', error);

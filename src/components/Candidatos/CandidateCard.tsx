...
export function CandidateCard({
  ...
  return <Card className={cn("hover-lift card-shadow bg-white border border-[#E5E7EB] overflow-hidden rounded-xl", !isAvailable && "opacity-70")}>
      <CardHeader className="pb-2 pt-3 px-3">
        <div className="flex items-start gap-2 mb-2">
          ...
          <div className="flex flex-col gap-1.5 flex-1 min-w-0">
            <Badge variant="outline" className={cn("text-sm font-bold rounded-full px-3 py-1 w-fit", statusColors[candidato.status] || statusColors["Banco de Talentos"])}>
              {statusIcons[candidato.status] || "⚪"} {candidato.status}
            </Badge>
            
            {candidato.vaga_titulo && <Badge variant="outline" className="text-sm font-medium px-3 py-1 w-fit text-[#00141D] border-[#faec3e] rounded-lg bg-[s#] bg-[#ffcd00]/[0.24]">
                📋 {candidato.vaga_titulo}
              </Badge>}
          </div>

          <div className="flex gap-1 flex-shrink-0">
            {candidato.disponibilidade_status === 'disponível' ? <Badge className="text-sm font-bold rounded-full px-2.5 py-1 bg-[#C9F4C7] text-[#1B5E20] hover:bg-[#C9F4C7]/90">
                ✅
              </Badge> : candidato.disponibilidade_status === 'não_disponível' ? <Badge className="text-sm font-bold rounded-full px-2.5 py-1 bg-[#FFD6D6] text-[#B71C1C] hover:bg-[#FFD6D6]/90">
                ❌
              </Badge> : null}
          </div>
        </div>
      ...
      <CardContent className="space-y-2 px-3 pb-3">
        {(candidato.nivel || candidato.area) && <div className="flex items-center gap-2 text-sm text-[#00141D]/80 font-medium">
        ...
        {(candidato.cidade || candidato.estado) && <div className="flex items-center gap-2 text-sm text-[#00141D]/80 font-medium">
        ...
        {candidato.recrutador && <div className="flex items-center gap-2 text-sm text-[#00141D]/80 font-medium">
          ...
          <div className="flex gap-1 pt-2 mt-2 border-t border-[#E5E7EB]">
            <Tooltip>
              <TooltipTrigger asChild>
                <Button variant="ghost" size="sm" className="h-9 flex-1 hover:bg-[#F9EC3F]/20 hover:text-[#00141D] font-medium px-2" onClick={e => {
                e.stopPropagation();
                onView();
              }}>
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <Button variant="ghost" size="sm" className="h-9 flex-1 text-destructive hover:text-destructive hover:bg-destructive/10 font-medium px-2" onClick={e => {
                e.stopPropagation();
                onDelete();
              }}>

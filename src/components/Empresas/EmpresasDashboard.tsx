...
const pipelineStages = [{
  ...
  label: "Acompanhamento 30/60/90"
}];
export function EmpresasDashboard({
  empresas
}: EmpresasDashboardProps) {
  ...
  return <div className="space-y-4">
      ...
      <Card className="p-4 border-gray-300 shadow-md">
        <h3 className="font-bold text-base text-foreground mb-3 flex items-center gap-2">
          <TrendingUp className="h-4 w-4 text-[#ffcd00]" />
          Pipeline
        </h3>
        <div className="space-y-2">
          {pipelineCounts.map(stage => <div key={stage.slug} className="flex items-center justify-between">

...
export function EmpresaInfoCard({ empresa }: EmpresaInfoCardProps) {
  ...
  const enderecoCompleto = [endereco, localizacao, empresa.cep]
    .filter(Boolean)
    .join(" | ");
    ...
    <Card className="border border-gray-300 shadow-md">
      ...
      <CardContent className="space-y-1">
        ...
        {empresa.razao_social && empresa.razao_social !== empresa.nome && (
          <InfoRow icon={Building2} label="Razão Social" value={empresa.razao_social} />
        )}
        {empresa.nome_fantasia && empresa.nome_fantasia !== empresa.nome && (
          <InfoRow icon={Building2} label="Nome Fantasia" value={empresa.nome_fantasia} />
        )}

...
export function RejectedCandidatesCard() {
  ...
  const loadCount = async () => {
    try {
      ...
      const { data: candidatosReprovados, error: candError } = await supabase
        .from('candidatos')
        .select('id')
        .or('status.eq.Reprovado rhello,status.eq.Reprovado Solicitante')
        .is('deleted_at', null);
  ...
  return (
    <>
      <Card 
        ...
        aria-label={`Candidatos reprovados sem WhatsApp (${count} candidatos)`}
        title="Candidatos que foram reprovados e ainda não receberam a mensagem de reprovação via WhatsApp"
      >
            ...
            <div className="space-y-2 flex-1">
              <div className="flex items-center gap-2">
                <p className="text-sm font-medium text-muted-foreground">
                  Candidatos Reprovados sem WhatsApp
                </p>
                {count > 0 && (
              ...
              <p className="text-[13px] text-muted-foreground">
                Candidatos em "Reprovado rhello" ou "Reprovado Solicitante" sem template de reprovação
              </p>

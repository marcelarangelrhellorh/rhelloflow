...
export function Column({
  ...
  return <div ref={setNodeRef} className={cn("flex flex-col min-w-[320px] h-full rounded-lg transition-all duration-200", "border border-gray-200 shadow-sm bg-white", isOver && "border-2 border-[#FFCD00] ring-2 ring-[#FFCD00]/20")} style={{
    backgroundColor: stage.color.columnBg
  }}>
      {/* Column Header */}
      <div className="flex items-center justify-between p-4 border-b border-gray-200 backdrop-blur-sm rounded-t-lg bg-neutral-50">
        <div className="flex items-center gap-2">
          <div className="w-2 h-2 rounded-full" style={{
          backgroundColor: stage.color.text
        }} />
          <h3 className="font-semibold text-sm text-foreground">{stage.name}</h3>
        </div>
        <Badge variant="secondary" className="text-xs font-medium" style={{

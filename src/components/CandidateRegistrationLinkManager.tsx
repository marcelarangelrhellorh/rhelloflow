...
import { Copy, ExternalLink, Plus, Trash2, Eye, Link2, Calendar, Users, Shield } from "lucide-react";
import { format } from "date-fns";
import { ptBR } from "date-fns/locale";
import { logger } from "@/lib/logger";

interface CandidateRegistrationLink {
  id: string;
  token: string;
  active: boolean;
...
export function CandidateRegistrationLinkManager() {
  const [links, setLinks] = useState<CandidateRegistrationLink[]>([]);
  ...
  const loadLinks = async () => {
    try {
      ...
      toast.error("Erro ao carregar links de cadastro de candidato");
    } finally {
      setLoading(false);
    }
  };
  ...
  const createLink = async () => {
    try {
      setCreating(true);
      const { data, error } = await supabase.functions.invoke("generate-candidate-registration-link", {
        body: {
  ...
  const copyLink = (token: string) => {
    const url = `${window.location.origin}/cadastro-candidato/${token}`;
  ...
  const getStatusBadge = (link: CandidateRegistrationLink) => {
    if (!link.active) {
      ...
      <CardHeader className="pb-4">
        <div className="flex items-center justify-between gap-4">
          <div className="space-y-1">
            <CardTitle className="text-lg">Links de Cadastro de Candidato</CardTitle>
            <CardDescription className="text-sm">
              Gerencie links públicos para cadastro de candidatos. Ao se inscrever, nosso time irá alocá-lo para a vaga que melhor se encaixa.
            </CardDescription>
          </div>
          <Dialog open={createDialogOpen} onOpenChange={setCreateDialogOpen}>
            ...
            <DialogContent>
              <DialogHeader>
                ...
                <DialogDescription>
                  Configure as opções do link de cadastro de candidato
                </DialogDescription>
              </DialogHeader>
              <div className="space-y-4 pt-4">
                ...
                <div>
                  ...
                  <Textarea 
                    id="note" 
                    placeholder="Ex: Link para divulgação em redes sociais" 
      ...
      <CardContent className="space-y-4">
        {links.length === 0 ? (
          <div className="text-center py-8 text-muted-foreground">
            ...
            <p className="text-sm">Crie um link para receber cadastros de candidatos.</p>
          </div>
        ) : (
          links.map(link => (
            <div key={link.id} className="border rounded-lg p-4 space-y-3 hover:bg-muted/50 transition-colors">
              ...
              <div className="flex items-center gap-2">
                <Input 
                  readOnly 
                  value={`${window.location.origin}/cadastro-candidato/${link.token}`} 

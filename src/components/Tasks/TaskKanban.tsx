...
export default function TaskKanban({
  ...
  return <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
      ...
      return <div key={column.status} className="flex flex-col gap-4" onDragOver={handleDragOver} onDrop={e => handleDrop(e, column.status)}>
            <Card className={`p-4 border-t-4 ${column.color}`}>
              <div className="flex items-center justify-between mb-4">
                <h3 className="font-bold text-lg">{column.label}</h3>
                <span className="text-muted-foreground bg-muted px-2 py-1 rounded text-base font-semibold">
                  {columnTasks.length}
                </span>
              </div>
            </Card>

...
export function CandidateMeetingsCard({ candidateId, candidateName, className }: CandidateMeetingsCardProps) {
  ...
  const overdueMeetings = pendingMeetings.filter(m => 
    m.due_date && isPast(new Date(m.due_date)) && !isToday(new Date(m.due_date))
  );
      ...
      <div className={cn("flex flex-col gap-3 rounded-lg p-6 bg-white dark:bg-background-dark border border-transparent shadow-sm", className)}>
        ...
        <div className="flex items-center gap-3">
          ...
          {overdueMeetings.length > 0 && (
            <Badge variant="destructive" className="text-base gap-1 whitespace-nowrap px-[10px]">
                ...
                >
                  <StatusIcon
                    className={cn(
                      ...
                      meeting.status === "in_progress" ? "text-blue-500" :
                  ...
                  {meeting.due_date && (
                    <span className={cn("text-sm font-medium", overdue ? "text-red-600" : "text-muted-foreground")}>
                      {format(new Date(meeting.due_date), "dd/MM HH:mm", { locale: ptBR })}
                    </span>
                  )}
                </div>
              );

...
export function ScorecardEvaluation({ candidateId, candidateName, vagaId }: ScorecardEvaluationProps) {
  ...
  function updateEvaluation(criteriaId: string, field: "score" | "notes", value: number | string) {
    setEvaluations((prev) =>
      prev.map((ev) =>
        ev.criteria_id === criteriaId ? { ...ev, [field]: value } : ev
      )
    );
  }

  function calculateScore() {
    if (criteria.length === 0 || evaluations.length === 0) return { total: 0, percentage: 0 };
      ...
      <CardContent className="space-y-6">
        ...
        {!loading && criteria.length > 0 && (
          <>
                    ...
                    <CardContent className="pt-4 space-y-3">
                              ...
                              <Tooltip>
                                <TooltipTrigger asChild>
                                  <Button
                                    ...
                                    size="sm"
                                    onClick={() =>
                                      updateEvaluation(criterion.id, "score", score)
                                    }
                                    className={cn(
                                      "flex-1",
                                      evaluation.score >= score && "bg-[#FFCD00] hover:bg-[#FAEC3E] text-[#00141D]"
                                    )}
                                  >
                                    <Star
                                      className={cn(
                                        "h-4 w-4",
                                        evaluation.score >= score && "fill-current"
                                      )}
                                    />
                                  </Button>
                                </TooltipTrigger>
                      ...
                      <div className="space-y-2">
                        ...
                        <Textarea
                          ...
                          onChange={(e) =>
                            updateEvaluation(criterion.id, "notes", e.target.value)
                          }
            ...
            {/* Score Summary */}
            {allScoresSet && (
              <Card className="border-2 border-[#FFCD00] bg-[#FFFDF6]">
            ...
            <div className="flex gap-2 pt-4 border-t">
              <Button
                onClick={handleSave}
                disabled={saving || !allScoresSet || !recommendation}
                className="flex-1 bg-[#FFCD00] hover:bg-[#FAEC3E] text-[#00141D]"
              >

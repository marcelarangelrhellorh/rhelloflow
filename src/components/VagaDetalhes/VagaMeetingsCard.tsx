  ...
  const {
    ...
    queryFn: async () => {
      const {
        data,
        error
      } = await supabase.from("tasks").select("*").eq("vaga_id", vagaId).eq("task_type", "meeting").order("due_date", {
        ascending: true
      });
  ...
  return <>
      <div className={cn("flex flex-col gap-3 rounded-lg p-6 bg-white dark:bg-background-dark border border-gray-200 dark:border-secondary-text-light/20 shadow-sm", className)}>
        <div className="flex items-center justify-between">
          ...
          <Button size="sm" variant="outline" onClick={handleNewMeeting} className="h-8 gap-1 px-4 text-sm font-semibold bg-[#00141D] text-white hover:bg-[#00141D]/90 border-[#00141D] rounded-full">
        ...
        {meetings.length === 0 && <p className="text-sm text-amber-600 dark:text-amber-500">
      ...
      <MeetingModal open={modalOpen} onClose={handleCloseModal} task={editingMeeting} defaultVagaId={vagaId} />
    </>;
}
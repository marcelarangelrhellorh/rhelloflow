...
export default function Vagas() {
  ...
  const loadVagas = async () => {
    try {
      ...
      const uniqueRecrutadores = Array.from(
        new Set(vagasWithCounts.map((v) => v.recrutador).filter(Boolean))
      ) as string[];
      const uniqueCS = Array.from(
        new Set(vagasWithCounts.map((v) => v.cs_responsavel).filter(Boolean))
      ) as string[];
      const uniqueClientes = Array.from(
        new Set(vagasWithCounts.map((v) => v.empresa).filter(Boolean))
      ) as string[];
  ...
  const filteredVagas = vagas.filter((vaga) => {
    ...
    const matchesRecrutador =
      recrutadorFilter === "all" || vaga.recrutador === recrutadorFilter;
    ...
    const matchesAttention =
      !attentionFilter || attentionIds.includes(vaga.id);
  ...
  const sortedVagas = [...filteredVagas].sort((a, b) => {
    if (ordenacao === "recentes") {
      ...
      const prioridadeOrder: { [key: string]: number } = {
        Alta: 1,
        Média: 2,
        Baixa: 3,
      };
      return (
        (prioridadeOrder[a.prioridade || "Baixa"] || 3) -
        (prioridadeOrder[b.prioridade || "Baixa"] || 3)
      );
    }
    return 0;
  });
  ...
  const handleMoveJob = async (
    ...
    setVagas((prev) =>
      prev.map((j) =>
        j.id === jobId
          ? { ...j, status: newStage.name, status_slug: newStage.slug, status_order: newStage.order }
          : j
      )
    );
    ...
    try {
      ...
      setVagas((prev) =>
        prev.map((j) =>
          j.id === jobId
            ? { ...j, status: oldStatusLabel, status_slug: oldStatusSlug }
            : j
        )
      );
    }
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            ...
            <p className="text-muted-foreground mt-1">
              {viewType === "cards" 
                ? "Gerencie todas as vagas abertas"
                : "Visualize o pipeline completo de vagas"}
            </p>
          </div>
          <div className="flex gap-3">
            <Button
              onClick={() => navigate("/vagas/nova")}
              className="bg-[#faec3e] text-slate-950 hover:bg-[#faec3e]/90 h-11 px-6"
            >

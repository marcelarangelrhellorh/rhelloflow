...
export default function FunilCandidatos() {
  ...
  async function loadVagas() {
    try {
      const {
        data,
        error
      } = await supabase.from("vagas").select("id, titulo, empresa, recrutador_id").not("status", "in", '("Concluído","Cancelada")').order("titulo");
  ...
  async function handleDragEnd(event: DragEndEvent) {
    ...
    try {
      ...
      if (error) throw error;
      setCandidatos(prev => prev.map(candidato => candidato.id === candidatoId ? {
        ...candidato,
        status: newStatus
      } : candidato));
      ...
      if (candidato?.vaga_relacionada_id && etapasImportantes.includes(newStatus)) {
        const resultado = newStatus.includes("Reprovado") ? "Reprovado" : newStatus.includes("Aprovado") ? "Aprovado" : "Contratado";
  ...
  const getCandidatosByStatus = (status: StatusCandidato) => {
    return candidatos.filter(candidato => {
      ...
      const matchesSearch = !searchQuery || candidato.nome_completo.toLowerCase().includes(searchQuery.toLowerCase()) || candidato.email.toLowerCase().includes(searchQuery.toLowerCase());
  ...
  const clientes = Array.from(new Set(candidatos.map(c => c.vaga?.empresa).filter(Boolean))) as string[];
  const recrutadoresVaga = Array.from(new Set(candidatos.map(c => c.vaga?.recrutador_id).filter(Boolean))) as string[];

  // Count active candidates (excluding final statuses and banco de talentos)
  const activeCandidatesCount = candidatos.filter(c => c.status !== "Contratado" && c.status !== "Reprovado Rhello" && c.status !== "Reprovado Solicitante" && c.status !== "Banco de Talentos").length;

  // Count candidates in banco de talentos
  const candidatosBancoTalentos = candidatos.filter(c => c.status === "Banco de Talentos").length;
    ...
    backgroundColor: '#FFFBF0'
      ...
      <div className="sticky top-0 z-20 bg-white border-b border-gray-200 shadow-sm">
        <div className="px-6 py-4 space-y-3">
          ...
          {/* Stats */}
          <StatsHeader totalCandidatosAtivos={activeCandidatesCount} candidatosBancoTalentos={candidatosBancoTalentos} />

          {/* Filters */}
          <FilterBar searchQuery={searchQuery} onSearchChange={setSearchQuery} vagaFilter={vagaFilter} onVagaChange={setVagaFilter} clienteFilter={clienteFilter} onClienteChange={setClienteFilter} recrutadorVagaFilter={recrutadorVagaFilter} onRecrutadorVagaChange={setRecrutadorVagaFilter} recrutadorFilter={recrutadorFilter} onRecrutadorChange={setRecrutadorFilter} vagas={vagas} clientes={clientes} recrutadoresVaga={recrutadoresVaga} recrutadores={recrutadores} users={users} />
        </div>
      </div>
      ...
      <DndContext sensors={sensors} onDragStart={handleDragStart} onDragEnd={handleDragEnd} collisionDetection={closestCenter}>
        <div className="overflow-x-auto pb-4">
          ...
          minWidth: 'max-content'
            ...
            return <FunnelColumn key={status} status={status} count={candidatosInColumn.length} colorClass={statusColors[status]} isOver={dragOverColumn === status} onDragOver={() => setDragOverColumn(status)} onDragLeave={() => setDragOverColumn(null)}>
                  {candidatosInColumn.length === 0 ? <div className="flex flex-col items-center justify-center py-8 text-center">
        ...
        <DragOverlay>
          {activeCandidato && <div className="w-[320px]">
              <CandidateFunnelCard candidato={activeCandidato} onDragStart={() => {}} isDragging />
            </div>}
        </DragOverlay>
      </DndContext>
    </div>;

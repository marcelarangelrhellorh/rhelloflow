...
export default function AuditLog() {
  ...
  const loadAuditEvents = async () => {
    try {
      setLoading(true);
      let query = supabase.from("audit_events").select("*").order("timestamp_utc", {
        ascending: false
      }).limit(500);
      if (dateFrom) {
  ...
  const verifyIntegrity = async () => {
    try {
      ...
      const {
        data,
        error
      } = await supabase.rpc("verify_audit_chain", {
      ...
      if (result.is_valid) {
        ...
        toast.error(`Falha na verificação de integridade: ${result.invalid_count} eventos inválidos encontrados`, {
          duration: 5000
        });
      }
    } catch (error) {
  ...
  const exportAuditLog = () => {
    const csv = [["Data/Hora", "Ator", "Ação", "Tipo de Recurso", "ID do Recurso", "Detalhes"], ...filteredEvents.map(e => [format(new Date(e.timestamp_utc), "yyyy-MM-dd HH:mm:ss"), e.actor.display_name, e.action, e.resource.type, e.resource.id || "-", JSON.stringify(e.payload || {})])].map(row => row.join(",")).join("\n");
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div className="relative">
              ...
              <Input placeholder="Pesquisar ator, ação, recurso..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="pl-9" />
            </div>
          ...
          <div className="flex gap-2 mt-4">
            ...
            {(dateFrom || dateTo || searchTerm || actionFilter !== "all") && <Button onClick={() => {
      ...
      <Card>
        ...
        <CardContent>
          {loading ? <div className="flex justify-center py-8">
              <div className="h-8 w-8 animate-spin rounded-full border-4 border-primary border-t-transparent" />
            </div> : filteredEvents.length === 0 ? <div className="text-center py-8 text-muted-foreground">
              Nenhum evento de auditoria encontrado
            </div> : <div className="space-y-4">
              {filteredEvents.map(event => <div key={event.id} className="border rounded-lg p-4 space-y-2 hover:bg-muted/50 transition-colors">
                  ...
                  {event.payload && Object.keys(event.payload).length > 0 && <details className="text-sm">
                  ...
                  {event.client?.user_agent && <div className="text-xs text-muted-foreground">

...
export default function GerenciarUsuarios() {
  ...
  useEffect(() => {
    if (clientFormData.company) {
      const filtered = availableCompanies.filter(company => company.toLowerCase().includes(clientFormData.company.toLowerCase()));
  ...
  const loadClients = async () => {
    try {
      ...
      const {
        data: clientRoles,
        error: rolesError
      } = await supabase.from("user_roles").select("user_id").eq("role", "client");
      ...
      const {
        data: clientUsers,
        error: usersError
      } = await supabase.from("users").select("*").in("id", clientIds).order("name");
  ...
  const loadAvailableJobs = async () => {
    try {
      const {
        data,
        error
      } = await supabase.from("vagas").select("id, titulo, empresa, status").order("titulo");
      if (error) throw error;
  ...
  const handleUpdateRoles = async (userId: string, selectedRoles: AppRole[]) => {
    try {
      ...
      if (selectedRoles.length > 0) {
        const {
          error: insertError
        } = await supabase.from("user_roles").insert(selectedRoles.map(role => ({
          user_id: userId,
          role: role
        })));
  ...
  const LinkJobDialog = ({
    ...
    return <Dialog open={open} onOpenChange={setOpen}>
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-3 py-2">
            <div className="space-y-1.5">
              ...
              <Select value={selectedJobId} onValueChange={setSelectedJobId}>
                ...
                <SelectContent>
                  {unlinkedJobs.map(job => <SelectItem key={job.id} value={job.id}>
          ...
          }} disabled={!selectedJobId} className="w-full h-9">
              Vincular
            </Button>
          </div>
        </DialogContent>
          ...
          <div className="space-y-3 py-2">
            ...
            <div className="space-y-2">
              <div className="flex items-center space-x-2">
                <Checkbox id={`role-admin-${user.id}`} checked={selectedRoles.includes("admin")} onCheckedChange={() => toggleRole("admin")} />
              ...
              <div className="flex items-center space-x-2">
                <Checkbox id={`role-recrutador-${user.id}`} checked={selectedRoles.includes("recrutador")} onCheckedChange={() => toggleRole("recrutador")} />
              ...
              <div className="flex items-center space-x-2">
                <Checkbox id={`role-cs-${user.id}`} checked={selectedRoles.includes("cs")} onCheckedChange={() => toggleRole("cs")} />
            ...
            <Button onClick={() => handleUpdateRoles(user.id, selectedRoles)} disabled={selectedRoles.length === 0} className="w-full h-9">
              Salvar Permissões
            </Button>
          </div>
        </DialogContent>
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-3 py-2">
            <div className="space-y-1.5">
              ...
              <Input id="new-password" type="password" minLength={6} value={newPassword} onChange={e => setNewPassword(e.target.value)} placeholder="Mínimo 6 caracteres" className="h-9" />
            </div>
            <Button onClick={handleSubmit} disabled={newPassword.length < 6} className="w-full h-9">
              Redefinir Senha
            </Button>
          </div>
        </DialogContent>
  ...
  return <div className="min-h-screen bg-background p-4 md:p-6">
      <div className="mx-auto max-w-7xl">
        ...
        {!showAddForm && <Button onClick={() => setShowAddForm(true)} variant="outline" className="w-full h-9 text-base font-semibold">
            <UserPlus className="mr-2 h-4 w-4" />
            Adicionar Novo Usuário rhello
          </Button>}

        {showAddForm && <Card className="border-primary/20">
              ...
              <form onSubmit={handleSubmit} className="space-y-3">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  ...
                  <div className="space-y-1.5">
                    ...
                    })} placeholder="joao@empresa.com" className="h-9" />
                  </div>

                  <div className="space-y-1.5">
                    <Label htmlFor="password" className="text-sm">Senha Temporária</Label>
                    <Input id="password" type="password" required minLength={6} value={formData.password} onChange={e => setFormData({
                      ...formData,
                      password: e.target.value
                    })} placeholder="Mínimo 6 caracteres" className="h-9" />
                  </div>

                  <div className="space-y-1.5">
                    <Label htmlFor="role" className="text-sm">Função</Label>
                    <Select value={formData.role} onValueChange={(value: any) => setFormData({
                      ...formData,
                      role: value
                    })}>
                      <SelectTrigger className="h-9">
        ...
        <Card>
          ...
          <CardContent>
            {loading ? <div className="text-center py-6 text-sm text-muted-foreground">Carregando...</div> : internalUsers.length === 0 ? <div className="text-center py-6 text-sm text-muted-foreground">
                Nenhum usuário cadastrado
              </div> : <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
                {internalUsers.map(user => <div key={user.id} className="flex items-center gap-3 p-3 border border-border rounded-lg hover:bg-accent/50 transition-colors">
                      ...
                      <AlertDialog>
                        ...
                        <AlertDialogContent>
                          ...
                          <AlertDialogFooter>
                            ...
                            <AlertDialogAction onClick={() => handleDeleteUser(user.id, user.name)} className="bg-destructive text-destructive-foreground hover:bg-destructive/90">
                              Excluir
                            </AlertDialogAction>
                          </AlertDialogFooter>
                        </AlertDialogContent>
                      </AlertDialog>
                      <Switch id={`active-${user.id}`} checked={user.active} onCheckedChange={() => toggleUserStatus(user.id, user.active)} className="scale-75" />
                    </div>
                  </div>)}
              </div>}
          </CardContent>
        ...
        <Card className="bg-primary/5 border-primary/20">
          <CardContent className="pt-4 pb-4">
            <div className="flex gap-2.5">
              ...
              <div className="space-y-1.5 text-xs">
                <p className="font-semibold text-primary text-xl">Sobre os Usuários rhello</p>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-1 text-muted-foreground">
                  <p className="text-base font-semibold"><strong>Recrutador:</strong> Gerencia vagas e candidatos</p>
                  <p className="text-base font-medium"><strong>CS:</strong> Mesmos acessos que recrutador</p>
                  <p className="text-base font-medium"><strong>Administrador:</strong> Acesso completo ao sistema</p>
                  <p className="text-base font-medium"><strong>Múltiplos Roles:</strong> Um usuário pode ter várias funções</p>
                </div>
              </div>
            </div>
          </CardContent>
          ...
          <TabsContent value="clients" className="space-y-4">
            ...
            {showClientForm && <Card className="border-primary/20">
                <CardHeader className="pb-3">
                  ...
                  <form onSubmit={handleClientSubmit} className="space-y-3">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      <div className="space-y-1.5">
                        ...
                        <Input id="client-name" required value={clientFormData.name} onChange={e => setClientFormData({
                      ...clientFormData,
                      name: e.target.value
                      ...
                      <div className="relative space-y-1.5">
                        ...
                        <Input id="client-company" required value={clientFormData.company} onChange={e => setClientFormData({
                      ...clientFormData,
                      company: e.target.value
                    })} onFocus={() => setShowCompanySuggestions(true)} onBlur={() => setTimeout(() => setShowCompanySuggestions(false), 200)} placeholder="Digite o nome da empresa" autoComplete="off" className="h-9 text-xl" />
                        {showCompanySuggestions && filteredCompanies.length > 0 && <div className="absolute z-50 w-full mt-1 bg-card border border-border rounded-md shadow-lg max-h-48 overflow-auto">
                            {filteredCompanies.map((company, index) => <button key={index} type="button" className="w-full text-left px-3 py-2 hover:bg-muted transition-colors text-sm" onClick={() => {
                        setClientFormData({
                          ...clientFormData,
                          company
                        });
                      ...
                      <div className="space-y-1.5">
                        ...
                        <Input id="client-email" type="email" required value={clientFormData.email} onChange={e => setClientFormData({
                      ...clientFormData,
                      email: e.target.value
                      ...
                      <div className="space-y-1.5">
                        ...
                        <Input id="client-password" type="password" required minLength={6} value={clientFormData.password} onChange={e => setClientFormData({
                      ...clientFormData,
                      password: e.target.value
                    })} placeholder="Mínimo 6 caracteres" className="h-9" />
                      </div>
                          ...
                          <div className="flex items-center gap-1 shrink-0">
                            ...
                            <AlertDialog>
                              ...
                              <AlertDialogContent>
                                ...
                                <AlertDialogFooter>
                                  <AlertDialogCancel>Cancelar</AlertDialogCancel>
                                  <AlertDialogAction onClick={() => handleDeleteUser(client.id, client.name)} className="bg-destructive text-destructive-foreground hover:bg-destructive/90">
                                    Excluir
                                  </AlertDialogAction>
                                </AlertDialogFooter>
                              </AlertDialogContent>
                            </AlertDialog>
                            <Switch id={`active-${client.id}`} checked={client.active} onCheckedChange={() => toggleUserStatus(client.id, client.active)} className="scale-75" />
                          </div>
                        </div>
